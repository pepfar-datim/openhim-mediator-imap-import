...
Install via npm
```
npm install -g openhim-mediator-imap-import
```
...
```
mkdir /opt/openhim-imap-import
```
Next get a copy of the default config and place it in an appropriate location, e.g.
```
mkdir /etc/openhim
wget https://raw.githubusercontent.com/jembi/openhim-mediator-imap-import/master/config/default.json
mv default.json /etc/openhim/mediator-imap-import.json
```
Edit the values in this config according to your setup.

Run the mediator:
```
openhim-mediator-imap-import --conf /etc/openhim/mediator-imap-import.json
```
...
With the script in place we can setup the channel and mediator in console. Go to *Mediators* and click on the *OpenHIM 


Script Mediator* item. From there click on the blue *Configure Mediator* button.
...
```
git clone https://github.com/jembi/openhim-mediator-imap-import.git
npm install
```
